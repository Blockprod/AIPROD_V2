...
class RunwayTTSAdapter(AudioAdapter):
    ...
    def __init__(
        ...
        self._model = (
            model if model is not None else os.environ.get("RUNWAY_AUDIO_MODEL", DEFAULT_MODEL)
        )
        self._voice = (
            voice if voice is not None else os.environ.get("RUNWAY_TTS_VOICE", DEFAULT_VOICE)
        )
